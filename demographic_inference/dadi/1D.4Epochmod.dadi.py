...
@author: annabelbeichman. Modified by Sergio Nigenda for vaquita project.
"""
...
modelName="1D.4Epochmod"

############### Parse input arguments ########################

parser = argparse.ArgumentParser(description='Infer a '+ modelName +' model from a 1D folded SFS in dadi')
parser.add_argument("--runNum",required=True,help="iteration number (e.g. 1-100)")
parser.add_argument("--pop",required=True,help="population identifier, e.g. 'ENP/GOC' or 'Vaquita'")
...
# usage:
# python 1D.Bottleneck.dadi.py --runNum $i --pop Vaquita --mu 1.08E-08 --L 481881665 --sfs [path to sfs] --outdir [path to outdir] Note: Please see the wrapper script were this line is incorporated to run this script from the wrapper
...
# Not fixing the event durantions
...
upper_bound = [10, 10, 10, 6, 6, 6]
lower_bound = [1e-4, 1e-4, 1e-4, 1e-4, 1e-4, 1e-4]
p0 = [0.9,0.01,0.1,0.1,0.01,0.9] # initial parameters
